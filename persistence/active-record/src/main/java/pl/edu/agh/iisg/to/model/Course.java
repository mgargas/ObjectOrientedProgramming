...
public class Course {
    ...
    public boolean enrollStudent(final Student student) {
        ...
        int id;
        try {
            id = QueryExecutor.createAndObtainId(enrollStudentSql);
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Student> studentList() {
    	String findStudentListSql = String.format("SELECT student_id FROM student_course where course_id = %d;", this.id);
        List<Student> resultList = new LinkedList<>();
        try {
            ResultSet rs = QueryExecutor.read(findStudentListSql);
            while(rs.next()){
                Optional<Student> studentRes = Student.findById(rs.getInt("student_id"));
                studentRes.ifPresent(resultList::add);
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
