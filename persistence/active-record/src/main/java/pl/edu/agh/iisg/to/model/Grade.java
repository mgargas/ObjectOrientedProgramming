...
public class Grade {
    ...
    public static boolean gradeStudent(final Student student, final Course course, final float grade) {
    	String gradeStudentSql = String.format("INSERT INTO '%s' (grade, student_id, course_id) VALUES('%f', '%d', '%d');", TABLE_NAME, grade, student.id(), course.id());
    	int id;
        try {
            id = QueryExecutor.createAndObtainId(gradeStudentSql);
